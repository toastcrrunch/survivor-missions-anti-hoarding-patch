class UrbanMallMission extends SurvivorMissions		//by Xellose and Funkdoc
{	//UrbanMall is a copy of CityMall mission, it has equal functionality but uses different primary MissionBuilding 
	//Secondary MissionBuilding is Police Department in big towns
	...
	{		
		...
		{ 
			...
			{	
				...
				Print("[SMM] UrbanMall Mission : MissionBuilding is "+ MissionBuilding.GetType() +" at "+ m_MissionDescription[2] +" of "+ m_MissionDescription[1] +" @ "+ MissionBuilding.GetPosition() );
				break;
			}	
		}
		if ( !MissionBuilding ) Print("[SMM] UrbanMall Mission : City store couldn't be found. Mission rejected!");
		else
		{
	...
	{	
		...
		if ( m_MissionObjects )
		{
			Print("[SMM] UrbanMall Mission : Despawning "+ m_MissionObjects.Count() +" mission objects from old mission...");		
		...
		if ( m_MissionAIs )
		{
			...
			{
				Print("[SMM] UrbanMall Mission : Despawning "+ m_MissionAIs.Count() +" mission AI's from old mission...");
			...
			else Print("[SMM] UrbanMall Mission : No mission AI's to despawn.");
		}
	...
	{		
		//Spawn Lockpick 
		g_Game.CreateObjectEx( "Lockpick", m_MissionPosition, ECE_PLACE_ON_SURFACE );
		
		//new MissionObject after deleting orange bag	
		MissionObject = ItemBase.Cast( g_Game.CreateObjectEx( "MissionChest", RewardsPos2, ECE_PLACE_ON_SURFACE ));
		MissionObject.SetTakeable(false);
		...
		g_Game.GetCallQueue( CALL_CATEGORY_SYSTEM ).CallLater( g_Game.UpdatePathgraphRegionByObject, 1000, false, PoliceDepartment );
		...
		for ( int i=0; i < PoliceInventory.Count(); i++ )
		{
			...
			Object item = g_Game.CreateObjectEx( InventoryType, InventoryPos, ECE_PLACE_ON_SURFACE );			
			item.SetPosition( InventoryPos );
			item.SetDirection( InventoryDir );
			item.SetOrientation( item.GetOrientation() + InventoryOri );
		}
		
		Print("[SMM] UrbanMall Mission : Mission rewards spawned in reward container. Randomly selected loadout was "+selectedLoadout+"." );			
	}
	...
	{	
		...
		{	//Primary mission is selected
			...
			if ( m_ObjectList.Count() > 0)
			{
				for ( int i=0; i < m_ObjectList.Count(); i++)
				{
					...
					{
						Print("[SMM] UrbanMall Mission : BeforeSpawnCleanUp :: Object  " + FoundObject.ToString() + " at new mission position was deleted.");
			...
			{			
				...
				if ( worldData )
				{
					...
					Print("[SMM] UrbanMall Mission : Test Hour is "+ hour +", SunRiseHour: "+ SunRiseHour +", SonSetHour: "+ SunSetHour );
					...
					if ( hour >= SunSetHour || hour <= SunRiseHour ) 
					{
						...
						if ( !lamp )
						{
							Print("[SMM] UrbanMall Mission : ERROR :: Could not spawn mission light at " + lightPos);
							return;
						}
						...
						if ( !canister )
						{
							Print("[SMM] UrbanMall Mission : ERROR :: Could not attach SmallGasCanister to mission light.");
						}
						...
						{
							Print("[SMM] UrbanMall Mission : ERROR :: Mission light can not be switched on.");
						}
			...
			Print("[SMM] UrbanMall Mission : Survivor Mission "+ m_selectedMission +" :: "+ m_MissionName +" ...primary mission deployed!");
		}
		else
		{	//Secondary mission is selected
			...
			{			
				...
				if ( worldData )
				{
					...
					if ( hour >= SunSetHour || hour <= SunRiseHour ) 
					{
						...
						//Choose the Light position Here. 
						lightPos = MissionBuilding.ModelToWorld("-8.253 -5.565 -0.123");
						MissionLight = g_Game.CreateObjectEx("PortableGasLamp", lightPos, ECE_PLACE_ON_SURFACE);
			...
			Print("[SMM] UrbanMall Mission : Survivor Mission Extension "+ m_selectedMission +" :: "+ m_MissionName +" ...secondary mission deployed!");
		}
	}
	...
	{	
		...
		if ( !MissionBuilding ) Print("[SMM] UrbanMall Mission : Extended Mission : Missionbuilding couldn't be found in "+ SecondaryLoc +"." );		
		else
		{
			if ( m_MissionExtended )
			{
	...
	{	//When player enters mission target zone at primary mission
		...
		if ( EventsWorldData.GetBuildingsAtLoc("Land_City_PoliceStation", SecondaryLoc, ExtendedPosList ))
		{
			...
			for ( int i=0; i < m_ObjectList.Count(); i++ )
			{
				...
				{									 
					//Close & lock armory and cell door 
					Building PoliceDepartment = Building.Cast( FoundObject );
					for ( int j=0; j < 12; j++ )
					{
						if ( PoliceDepartment.IsDoorOpen(j) )	PoliceDepartment.CloseDoor(j);
						//11=cell, 9=armory
						if ( ( j == 9 || j == 11 ) && !PoliceDepartment.IsDoorLocked(j) )	PoliceDepartment.LockDoor(j);
					}
			...
			Print("[SMM] UrbanMall Mission : New Mission Position is Land_City_PoliceStation at "+ SecondaryLoc +" @"+ m_MissionPosition );
			SecondaryLoc.Replace("_"," ");
		}
		else Print("[SMM] UrbanMall Mission : ERROR :: Can't get secondary MissionPosition in "+ SecondaryLoc +" from EventsWorldData!");
		...
		{ 
			...
			{	
				...
				for ( j = 5 ; j < PriInfectSpawns.Count() ; j++ )
				{
		...
		Print("[SMM] UrbanMall Mission : "+ m_selectedMission +" :: "+ m_MissionName +" ...mission extended!");		
	}
							...
							if ( FoundFoodObjects >= ReqFoodAmount && FoundMedObjects >= ReqMedAmount )
							{
								...
								for ( int n = 0; n < m_ObjectList.Count(); n++)
								{
									...
									if ( FoundObject.IsMan() )
									{
										...
										Print("[SMM] UrbanMall Mission : Player with SteamID64: "+ player.GetIdentity().GetPlainId() +" has successfully stored the requested amount of "+ (ReqFoodAmount + ReqMedAmount) +" MissionObjects in the container.");
										break;
									}
								}
								...
								for ( int m=0; m < AssistingPlayers.Count(); m++)
								{
									...
									//log player
									Print("[SMM] UrbanMall Mission : Player with SteamID64: "+ IdentPlainID +" deserves respect for his "+ ExtMsg +".");
	...
	{	//When first player enters the mission zone (primary/secondary)
		...
		{			
			...
			{ 
				...
				{			 
					MissionBuilding = FoundObject;
					Print("[SMM] UrbanMall Mission : Secondary mission zone triggered. MissionBuilding extended is Land_City_PoliceStation at "+ SecondaryLoc +" @ "+ MissionBuilding.GetPosition() );
		...
		{
			Print("[SMM] UrbanMall Mission : ERROR :: MissionBuilding can't be found!");
			if ( MissionSettings.DebugMode ) Print("[SMM] UrbanMall Mission : MissionBuilding is NULL!");
			return false;
		}
	}
}
