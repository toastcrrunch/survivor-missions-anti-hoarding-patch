class CityMallMission extends SurvivorMissions		//by Xellose and Funkdoc
{
	...
	{		
		...
		Barricades.Insert( new Param3<string,vector,vector>("Land_Roadblock_WoodenCrate", "-9.69 -0.882 1.7", "-32 0 0"));
		Barricades.Insert( new Param3<string,vector,vector>("Land_Roadblock_WoodenCrate", "-9.59 -0.262 1.7", "-34 0 5"));
		...
		{ 
			...
			{	
				...
				Print("[SMM] CityMall Mission : MissionBuilding is "+ MissionBuilding.GetType() +" at "+ m_MissionDescription[2] +" of "+ m_MissionDescription[1] +" @ "+ MissionBuilding.GetPosition() );
				break;
			}	
		}
		if ( !MissionBuilding ) Print("[SMM] CityMall Mission : City store couldn't be found. Mission rejected!");
		else
		{
	...
	{	
		...
		if ( m_MissionObjects )
		{
			Print("[SMM] CityMall Mission : Despawning "+ m_MissionObjects.Count() +" mission objects from old mission...");		
		...
		if ( m_MissionAIs )
		{
			...
			{
				Print("[SMM] CityMall Mission : Despawning "+ m_MissionAIs.Count() +" mission AI's from old mission...");
			...
			else Print("[SMM] CityMall Mission : No mission AI's to despawn.");
		}
	...
	{		
		if ( m_MissionExtended )
		{
			...
			{
				Object Med = g_Game.CreateObjectEx("TetracyclineAntibiotics", MissionBuilding.ModelToWorld("-5.836 -0.216 1.005"), ECE_PLACE_ON_SURFACE); //safe position
	...
	void SpawnRewards()
	{
		...
		Print("[SMM] CityMall Mission : Mission rewards spawned in reward container. Randomly selected loadout was "+selectedLoadout+"." );			
	}
		...
		{	
			...
			if ( m_ObjectList.Count() > 0)
			{
				for ( int i=0; i < m_ObjectList.Count(); i++)
				{
					...
					{
						Print("[SMM] CityMall Mission : BeforeSpawnCleanUp :: Object  " + FoundObject.ToString() + " at new mission position was deleted.");
			...
			{			
				...
				if ( worldData )
				{
					...
					Print("[SMM] CityMall Mission : Test Hour is "+ hour +", SunRiseHour: "+ SunRiseHour +", SonSetHour: "+ SunSetHour );
					...
					if ( hour >= SunSetHour || hour <= SunRiseHour ) 
					{
						...
						if ( !lamp )
						{
							Print("[SMM] CityMall Mission : ERROR :: Could not spawn mission light at " + lightPos);
							return;
						}
						...
						if ( !canister )
						{
							Print("[SMM] CityMall Mission : ERROR :: Could not attach SmallGasCanister to mission light.");
						}
						...
						{
							Print("[SMM] CityMall Mission : ERROR :: Mission light can not be switched on.");
						}
			...
			for ( int j=0; j < Barricades.Count(); j++ )
			{
				...
				PlankBarricade = g_Game.CreateObject( BarricadeType, BarricadePos, true );
				else
				{
			...
			EntityAI Cooker = EntityAI.Cast( g_Game.CreateObjectEx( "PortableGasStove", MissionBuilding.ModelToWorld("-5.96 -0.2156 0.84"), ECE_PLACE_ON_SURFACE ));
			...
			m_MissionObjects.Insert( g_Game.CreateObjectEx( "Paper", MissionBuilding.ModelToWorld("-9.74 -1.17 -7.49"), ECE_PLACE_ON_SURFACE ));
			m_MissionObjects.Insert( g_Game.CreateObjectEx( "Paper", MissionBuilding.ModelToWorld("-8.18 -1.17 -8.09"), ECE_PLACE_ON_SURFACE ));
			m_MissionObjects.Insert( g_Game.CreateObjectEx( "ShitGround", MissionBuilding.ModelToWorld("-9.54 -1.17 -7.69"), ECE_PLACE_ON_SURFACE ));
			m_MissionObjects.Insert( g_Game.CreateObjectEx( "ShitGround", MissionBuilding.ModelToWorld("-9.34 -1.17 -7.89"), ECE_PLACE_ON_SURFACE ));
			
			//update AI navmesh
			g_Game.GetCallQueue( CALL_CATEGORY_SYSTEM ).CallLater( g_Game.UpdatePathgraphRegionByObject, 1000, false, MissionBuilding );
			
			Print("[SMM] CityMall Mission : Survivor Mission "+ m_selectedMission +" :: "+ m_MissionName +" ...primary mission deployed!");
		}
		else
		{
			...
			Print("[SMM] CityMall Mission : Survivor Mission Extension "+ m_selectedMission +" :: "+ m_MissionName +" ...secondary mission deployed!");
		}
	}
	...
	{	
		...
		if ( !MissionBuilding ) Print("[SMM] CityMall Mission : Extended Mission : Missionbuilding couldn't be found in "+ SecondaryLoc +"." );		
		else
		{
			if ( m_MissionExtended )
			{
	...
	{	//When player enters mission target zone at primary mission
		...
		if ( EventsWorldData.GetBuildingsAtLoc("Land_Village_PoliceStation", SecondaryLoc, ExtendedPosList ))
		{
			...
			Print("[SMM] CityMall Mission : New Mission Position is Land_Village_PoliceStation at "+ SecondaryLoc +" @"+ m_MissionPosition );
			SecondaryLoc.Replace("_"," ");
		}
		else Print("[SMM] CityMall Mission : Can't get secondary MissionPosition in "+ SecondaryLoc +" from EventsWorldData!");
		...
		Print("[SMM] CityMall Mission : "+ m_selectedMission +" :: "+ m_MissionName +" ...mission extended!");
	}
							...
							if ( FoundFoodObjects >= ReqFoodAmount && FoundMedObjects >= ReqMedAmount )
							{
								...
								for ( int n = 0; n < m_ObjectList.Count(); n++)
								{
									...
									if ( FoundObject.IsMan() )
									{
										...
										Print("[SMM] CityMall Mission : Player with SteamID64: "+ player.GetIdentity().GetPlainId() +" has successfully stored the requested amount of "+ (ReqFoodAmount + ReqMedAmount) +" MissionObjects in the container.");
										break;
									}
								}
								...
								for ( int m=0; m < AssistingPlayers.Count(); m++)
								{
									...
									//log player
									Print("[SMM] CityMall Mission : Player with SteamID64: "+ IdentPlainID +" deserves respect for his "+ ExtMsg +".");
	...
	{	//When first player enters the mission zone (primary/secondary)
		...
		{			
			...
			{ 
				...
				{			 
					MissionBuilding = FoundObject;
					Print("[SMM] CityMall Mission : Secondary mission zone triggered. MissionBuilding extended is Land_Village_PoliceStation at "+ SecondaryLoc +" @ "+ MissionBuilding.GetPosition() );
		...
		{
			Print("[SMM] CityMall Mission : ERROR :: MissionBuilding can't be found!");
			if ( MissionSettings.DebugMode ) Print("[SMM] CityMall Mission : MissionBuilding is NULL!");
			return false;
		}
	}
}
