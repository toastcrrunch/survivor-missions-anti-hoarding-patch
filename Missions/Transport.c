class TransportMission extends SurvivorMissions		//by Xellose and Funkdoc
{
	...
	{		
		...
		{ 
			...
			{	
				...
				Print("[SMM] Transport Mission : MissionBuilding is "+ MissionBuilding.GetType() +" at "+ m_MissionDescription[2] +" of "+ m_MissionDescription[1] +" @ "+ MissionBuilding.GetPosition() );
				break;
			}	
		}
		if ( !MissionBuilding ) Print("[SMM] Transport Mission : Garage couldn't be found. Mission rejected!");
		else
		{
			...
			{ 
				...
				{				 
					Print("[SMM] Transport Mission : BeforeSpawnCleanup :: "+ FoundWreck.GetType() +" was found near the garage, but can't be deleted. It could block the garage door.");
	...
	{	
		...
		if ( m_MissionObjects )
		{
			Print("[SMM] Transport Mission : Despawning "+ m_MissionObjects.Count() +" mission objects from old mission...");		
		...
		if ( m_MissionAIs )
		{
			...
			{
				Print("[SMM] Transport Mission : Despawning "+ m_MissionAIs.Count() +" mission AI's from old mission...");
			...
			else Print("[SMM] Transport Mission : No mission AI's to despawn.");
		}
	...
	void SpawnCar()
	{
		...
		//Create MissionCar
		Print("[SMM] Transport Mission : Spawning mission vehicle");
	...
	{		
		...
		Print("[SMM] Transport Mission : Mission rewards spawned in reward container. Randomly selected loadout was "+selectedLoadout+"." );			
	}
		...
		{	
			...
			if ( m_ObjectList.Count() > 0)
			{
				for ( int i=0; i < m_ObjectList.Count(); i++)
				{
					...
					{
						Print("[SMM] Transport Mission : BeforeSpawnCleanUp :: Object  " + FoundObject.ToString() + " at new mission position was deleted.");
			...
			{			
				...
				if ( worldData )
				{
					...
					Print("[SMM] Transport Mission : Test Hour is "+ hour +", SunRiseHour: "+ SunRiseHour +", SonSetHour: "+ SunSetHour );
					...
					if ( hour >= SunSetHour || hour <= SunRiseHour ) 
					{
						...
						if ( !lamp )
						{
							Print("[SMM] Transport Mission : ERROR :: Could not spawn mission light at " + m_MissionPosition);
							return;
						}
						...
						if ( !canister )
						{
							Print("[SMM] Transport Mission : ERROR :: Could not attach SmallGasCanister to mission light.");
						}
						...
						{
							Print("[SMM] Transport Mission : ERROR :: Mission light can not be switched on.");
						}
			...
			Print("[SMM] Transport Mission : Survivor Mission "+ m_selectedMission +" :: "+ m_MissionName +" ...primary mission deployed!");
		}
		else
		{
			//check darkness for location light support and spawn lamp if needed
			...
			{			
				...
				if ( worldData )
				{
					...
					if ( hour >= SunSetHour || hour <= SunRiseHour ) 
					{
						...
						//Choose the Light position Here. 
						MissionLight = g_Game.CreateObjectEx("PortableGasLamp", MissionBuilding.ModelToWorld( "-1.998 -0.61 -0.012" ), ECE_PLACE_ON_SURFACE);
			...
			Print("[SMM] Transport Mission : Survivor Mission Extension "+ m_selectedMission +" :: "+ m_MissionName +" ...secondary mission deployed!");
		}
	}
	...
	{	
		...
		if ( !MissionBuilding ) Print("[SMM] Transport Mission : Extended Mission : Missionbuilding couldn't be found in "+ SecondaryLoc +"." );		
		else
		{
			if ( m_MissionExtended )
			{
	...
	{	//When player enters mission target zone at primary mission
		...
		if ( EventsWorldData.GetBuildingsAtLoc("Land_FuelStation_Build", SecondaryLoc, ExtendedPosList ))
		{
			...
			Print("[SMM] Transport Mission : New Mission Position is Land_FuelStation_Build at "+ SecondaryLoc +" @ "+ m_MissionPosition );
			SecondaryLoc.Replace("_"," ");
		}
		else Print("[SMM] Transport Mission : Can't get secondary MissionPosition in "+ SecondaryLoc +" from EventsWorldData!");
							...
							if ( FoundObjects >= ReqWeedAmount )
							{
								...
								for ( int n = 0; n < m_ObjectList.Count(); n++)
								{
									...
									if ( FoundObject.IsMan() )
									{
										...
										Print("[SMM] Transport Mission : Player with SteamID64: "+ player.GetIdentity().GetPlainId() +" has successfully stored the requested amount of "+ ReqWeedAmount +" MissionObjects in the container.");
										break;
									}
								}
								...
								for ( int m=0; m < AssistingPlayers.Count(); m++)
								{
									...
									//log player
									Print("[SMM] Transport Mission : Player with SteamID64: "+ IdentPlainID +" deserves respect for his "+ ExtMsg +".");
		...
		if ( !CarIsHijacked )
		{
			...
			else if ( CarPosition )
			{
				...
				for ( int k=0; k < m_ObjectList.Count(); k++)
				{
					...
					{
						Print("[SMM] Transport Mission : MissionCar was hijacked. Deleting car cover...");
						CarIsHijacked = true;
						g_Game.ObjectDelete( FoundObject );
	...
	{	//When first player enters the mission zone (primary/secondary)
		...
		{			
			...
			{ 
				...
				{			 
					MissionBuilding = FoundObject;
					Print("[SMM] Transport Mission : Secondary mission zone triggered. MissionBuilding extended is Land_FuelStation_Build at "+ SecondaryLoc +" @ "+ MissionBuilding.GetPosition() );
		...
		{
			Print("[SMM] Transport Mission : ERROR :: MissionBuilding can't be found!");
			if ( MissionSettings.DebugMode ) Print("[SMM] Transport Mission : MissionBuilding is NULL!");
			return false;
		}
	}
}
