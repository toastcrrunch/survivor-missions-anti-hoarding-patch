class ApartmentMission extends SurvivorMissions		//by Xellose and Funkdoc
{
	...
	{		
		...
		if ( m_MissionObjects )
		{
			Print("[SMM] Apartment Mission : Despawning "+ m_MissionObjects.Count() +" mission objects from old mission...");
		...
		if ( m_MissionAIs )
		{
			...
			{
				Print("[SMM] Apartment Mission : Despawning "+ m_MissionAIs.Count() +" mission AI's from old mission...");
			...
			else Print("[SMM] Apartment Mission : No mission AI's to despawn.");
		}
	...
	{	
		...
		Print("[SMM] Apartment Mission : Mission rewards spawned in reward container. Randomly selected loadout was "+ selectedLoadout +"." );
		...
		for ( int i=0; i < Barricades.Count(); i++ )
		{
			...
			Object PlankBarricade = g_Game.CreateObject( BarricadeType, BarricadePos, false, false, true );
		...
		//Object phObject = g_Game.CreateObjectEx("Land_Boat_Small1", MissionBuilding.ModelToWorld("0.360 -7.410 6.730"), ECE_KEEPHEIGHT|ECE_CREATEPHYSICS|ECE_UPDATEPATHGRAPH); //|ECE_LOCAL
		Object phObject = g_Game.CreateObject("Land_Boat_Small1", MissionBuilding.ModelToWorld("0.360 -7.410 6.730"), true );
		...
		{			
			...
			if ( worldData )
			{
				...
				Print("[SMM] Apartment Mission : Test Hour is "+ hour +", SunRiseHour: "+ SunRiseHour +", SonSetHour: "+ SunSetHour );
				
				if ( hour >= SunSetHour || hour <= SunRiseHour ) 
				{

					//lamp at body in room
					vector lightPos = MissionBuilding.ModelToWorld("-7.3545 6.1203 -5.6489");
					MissionLight = g_Game.CreateObjectEx("PortableGasLamp", lightPos, ECE_PLACE_ON_SURFACE);

					ItemBase lamp = ItemBase.Cast(MissionLight);
					if ( !lamp )
					{
						Print("[SMM] Apartment Mission : ERROR :: Could not spawn mission light at " + lightPos);
						return;
					}
					...
					if ( !canister )
					{
						Print("[SMM] Apartment Mission : ERROR :: Could not attach SmallGasCanister to mission light.");
					}
					...
					{
						Print("[SMM] Apartment Mission : ERROR :: Mission light can not be switched on.");
					}
					m_MissionObjects.Insert( lamp );
					
					//lamp at stairs
					lightPos = MissionBuilding.ModelToWorld("0.746 6.121 4.045");
					MissionLight = g_Game.CreateObjectEx("PortableGasLamp", lightPos, ECE_PLACE_ON_SURFACE);
		...
		Print("[SMM] Apartment Mission : Survivor Mission "+ m_selectedMission +" :: "+ m_MissionName +" ...mission deployed!");
	}	
	...
	{			
		...
		PlayerBase DeadSurvivor = PlayerBase.Cast(g_Game.CreatePlayer( null, "SurvivorM_Oliver", SurvivorPos, 0 , "Oliver"));
		...
		m_MissionAIs.InsertAt( g_Game.CreateObjectEx( "ZmbF_JournalistNormal_White", InfGirlPos, ECE_PLACE_ON_SURFACE | ECE_INITAI ), 1 );
	...
	{	//When player enters last mission target zone
		...
		for ( int m=0; m < AssistingPlayers.Count(); m++)
		{
			...
			//log player
			Print("[SMM] Apartment Mission : Player with SteamID64: "+ IdentPlainID +" deserves respect for his "+ ExtMsg +".");
		...
		g_Game.GetCallQueue( CALL_CATEGORY_SYSTEM ).CallLater( g_Game.UpdatePathgraphRegionByObject, 1000, false, Tenement );
		...
		Object rewardChest = m_MissionObjects[0];
	...
	{	//When first player enters the mission zone (primary/secondary)
		...
		{ 
			...
			{	
				...
				Print("[SMM] Apartment Mission : MissionBuilding is "+ MissionBuilding.GetType() +" at "+ m_MissionDescription[2] +" of "+ m_MissionDescription[1] +" @ "+ MissionBuilding.GetPosition() );
		...
		{
			Print("[SMM] Apartment Mission : ERROR :: MissionBuilding can't be found!");
			if ( MissionSettings.DebugMode ) Print("[SMM] Apartment Mission : MissionBuilding is NULL!");			
			return false;
		}
	}
}